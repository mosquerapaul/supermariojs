...
import Visible from '../traits/Visible.js';
...
class Behavior extends Trait {
    ...
    collides(us, them) {
        ...
        if(them.stomper) {
            if(us.vel.y < them.vel.y) {
                ...
                this.handleNodge(us, them);
            }
        }
    }

    handleNodge(us, them) { // Empujar concha
        if(this.state === STATE_WALKING) {
            ...
            const impact = travelDir * impactDir;
            if( impact === -1) {
                them.killable.kill();
            }
        }
    ...
    handleStomp(us, them) { // Matar al pisar
        if(this.state === STATE_WALKING) {
            this.hide(us);
        } else if(this.state === STATE_HIDING) {
            us.killable.kill();
